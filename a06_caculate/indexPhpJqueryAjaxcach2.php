...
<body>
    ...
    <script type="text/javascript">
        ...
        function onSubmitPost() {
            $.post("process_form_for_jquery_ajax.php", {
                    a: $('[name=a]').val(),
                    b: $('[name=b]').val(),
                    cal: $('[name=cal]').val(),
                },
        ...
        function onSubmitGet() {
            $.get("process_form_for_jquery_ajax.php?a=" + $('[name=a]').val() + "&b=" + $('[name=b]').val() + "&cal=" + encodeURIComponent($('[name=cal]').val()),
                function(data, status) {
                    console.log("data: " + data);
                    console.log("status: " + status);
                    $('#result2').val("=" + data);

                    // gan gia tri trc cho so thu nhat
                    $('[name=a]').val(data);
                    $('[name=b]').val('');
                    $('[name=cal]').val('');
                });
